...
for i in range(len(video_game_sales)):
    genre = video_game_sales[i][4]
    sales = video_game_sales[i][9]
    
    if genre in sales_by_genre:
        sales_by_genre[genre] += sales  # Clean and simple
    else:
        sales_by_genre[genre] = sales
print(sales_by_genre)
games_per_publisher ={}
for i in range(len(video_game_sales)):
    publisher = video_game_sales[i][5]
    if publisher in games_per_publisher:
        games_per_publisher[publisher] += 1  # Add 1 to the count if they exist
    else:
